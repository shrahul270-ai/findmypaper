...
import { 
  ...
  Settings, LogOut, BookOpen, MapPin, User, Store, ClipboardList, ShieldCheck, History, Menu, X 
} from 'lucide-react';
...
export default function Sidebar({ role }: { role: keyof typeof menuItems }) {
  ...
  return (
    <>
      {/* Mobile Top Header */}
      <div className="md:hidden flex items-center justify-between p-4 bg-white border-b border-slate-100 sticky top-0 z-[100]">
        <div className="flex items-center gap-2">
          ...
          <h1 className="font-black tracking-tighter text-slate-900 text-lg italic uppercase leading-none">FINDMYPAPER</h1>
        </div>
      ...
      {/* Sidebar Content */}
      <aside className={cn(
        "fixed inset-y-0 left-0 z-[120] w-64 bg-white border-r border-slate-100 flex flex-col transition-transform duration-300 md:relative md:translate-x-0 md:flex shrink-0 h-screen",
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto mt-4 md:mt-0">
          ...
          {items.map((item) => (
            <Link
              ...
              onClick={() => setIsOpen(false)}
              className={cn(
                "flex items-center gap-3 px-4 py-4 md:py-3 rounded-xl text-sm font-bold transition-all uppercase tracking-widest group",
                pathname === item.href
                  ? "bg-indigo-600 text-white shadow-xl shadow-indigo-100"
                  : "text-slate-400 hover:bg-slate-50 hover:text-slate-600"
              )}
            >
              <item.icon size={20} className={cn("transition-all", pathname === item.href ? "scale-110" : "group-hover:scale-110")} />
        ...
        <div className="p-4 border-t border-slate-50 space-y-2">
          ...
          <button 
            ...
            className="flex items-center gap-3 w-full px-4 py-4 text-rose-500 hover:bg-rose-50 rounded-xl transition-all text-[10px] font-black uppercase tracking-[0.2em] group"
          >
